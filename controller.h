...
public:
    virtual std::string addAstronaut(const std::string &type, const std::string &astronautName) override {
        ...
        else if (type == "геодезист" || type == "geodes")
            astronaut = new Geodesist(astronautName);
        else if (type == "метеорологист" || type == "meteorologist")
            astronaut = new Meteorologist(astronautName);
        else throw std::invalid_argument("Няма такъв вид астронавт!\n");

        this->astronauts->add(*astronaut);

        return astronautName + " беше добавен, длъжност - " + type + '\n';
    }
    ...
    virtual std::string retireAstronaut(const std::string &astronautName) override {
        auto astronautToRemove = std::find_if(this->astronauts->getModels().begin(), this->astronauts->getModels().end(),
    ...
    virtual std::string explorePlanet(const std::string &planetName) override {
        std::shuffle(this->astronauts->getModels().begin(), this->astronauts->getModels().end(), std::mt19937(std::random_device()()));
        ...
        auto deadAstr = std::count_if(suitableAstronauts.begin(), suitableAstronauts.end(),
                                      [](Astronaut *astronaut) { return !astronaut->canBreathe(); });
        return "Експедиция на планетата " + planetName + " завърши! Неоцелели астронавти - " +
               std::to_string(deadAstr) + '\n';
    }
